...
export default async function DashboardPage() {
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <section className="mt-8 grid gap-4 md:grid-cols-3">
      ...
      <div className="rounded-2xl border border-stone-200 bg-[#fff8e7] p-5"><CalendarClock className="h-5 w-5 text-amber-700" /><p className="mt-6 text-xs font-bold uppercase tracking-[0.14em] text-amber-800">Próxima revisão</p><p className="mt-2 text-lg font-semibold text-stone-950">{nextReview?.titulo ?? "Nenhuma agendada"}</p><p className="mt-1 text-sm text-stone-600">{nextReview ? new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short" }).format(nextReview.agendadaPara) : "Quando um resumo ficar pronto, três revisões entrarão na agenda."}</p><Link href="/app/revisoes" className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-amber-900">Abrir agenda <ArrowRight className="h-4 w-4" /></Link></div>
    </section>
    ...
    <section className="mt-10"><div className="flex items-end justify-between"><div><p className="text-sm font-bold uppercase tracking-[0.14em] text-stone-400">Para retomar</p><h2 className="mt-1 text-2xl font-semibold tracking-tight text-stone-950">Materiais recentes</h2></div>{materials.length > 0 && <Link href="/app/materiais" className="inline-flex items-center gap-1 text-sm font-semibold text-emerald-800">Ver todos <ArrowRight className="h-4 w-4" /></Link>}</div>
      {materials.length === 0 ? <div className="mt-5 rounded-2xl border border-dashed border-stone-300 bg-white/60 p-8 text-center"><FileText className="mx-auto h-6 w-6 text-stone-400" /><h3 className="mt-3 font-semibold text-stone-950">Seu acervo está vazio</h3><p className="mt-1 text-sm text-stone-600">Crie uma matéria e adicione um PDF para preparar seu resumo e suas palavras-chave.</p><Link href="/app/materiais" className="btn-secondary mt-5">Adicionar primeiro material</Link></div> : <div className="mt-5 grid gap-3">{materials.map((material) => <article key={material.id} className="flex items-center gap-4 rounded-2xl border border-stone-200 bg-white p-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-rose-50 text-rose-700"><FileText className="h-5 w-5" /></span><div className="min-w-0 flex-1"><Link href={material.materiaId ? `/app/materiais?materia=${material.materiaId}` : "/app/materiais"} className="block truncate font-semibold text-stone-900 hover:text-emerald-800">{material.titulo}</Link><p className="mt-0.5 text-sm text-stone-500">{material.plano?.titulo ?? "Sem plano"}</p><div className="mt-2 flex flex-wrap gap-1.5">{material.palavrasChave.map((tag) => <span key={tag.id} className="badge">{tag.termo}</span>)}</div></div></article>)}</div>}
    </section>
  </div>;
}
