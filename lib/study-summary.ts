...
const significant = (value: string) =>
  value.length >= 3 && !stopWords.has(normalize(value));
const sentenceSegmenter = new Intl.Segmenter("pt-BR", {
...
function isLikelyPersonName(tokens: string[]) {
  return (
    tokens.length >= 2 &&
    tokens.length <= 3 &&
    tokens.every((token) => /^[A-ZÁÉÍÓÚÂÊÔÃÕÇ][a-záéíóúâêôãõç]+$/u.test(token))
  );
}
  ...
  return pages.map((page) => ({
    ...
    text: page.text
      ...
      .filter((line) => {
        ...
        if (
          !trimmed ||
          /^(?:p[aá]g(?:ina)?\.?\s*)?\d+(?:\s*(?:\/|de)\s*\d+)?$/i.test(trimmed)
        )
          return false;
        if (
          ...
          return false;
        const count =
          recurring.get(normalize(trimmed).replace(/\d+/g, "#"))?.size ?? 0;
        return !(
          count >= 3 &&
...
export function summarizePages(pages: PdfPageText[], subject = "") {
  ...
  for (const page of clean) {
    for (const block of page.text.split(/\n+/).filter(Boolean)) {
      if (
        /\b(?:CEBRASPE|FGV|FCC|VUNESP|CESPE)\b|coment[aá]rio formulado pela banca/i.test(
          block,
        )
      )
        continue;
      ...
      for (const fragment of block.split(/[.,;:!?()[\]{}\n]/)) {
        ...
        for (let index = 0; index < tokens.length; index++) {
          ...
          if (tokens[index + 1] && significant(tokens[index + 1]))
            addTerm(
              tokens.slice(index, index + 2).join(" "),
              page.page,
              heading,
              2,
            );
          if (
            connectors.has(normalize(tokens[index + 1] ?? "")) &&
            tokens[index + 2] &&
            significant(tokens[index + 2])
          )
            addTerm(
              tokens.slice(index, index + 3).join(" "),
              page.page,
              heading,
              2,
            );
  ...
  for (const [key, term] of vocabulary) {
    ...
    term.score =
      recurrence *
      (1 + Math.log2(term.pages.size)) *
      (term.heading ? 1.4 : 1) *
      (context.has(key) ? 1.15 : 1);
    if (term.size > 1) {
      // Recurring phrases are more useful than an isolated pair of adjacent words.
      term.score *= term.count >= 2 || term.heading ? 1.9 : 0.45;
    }
  }
  const rankedTerms = [...vocabulary.entries()].sort(
    (a, b) => b[1].score - a[1].score || a[0].localeCompare(b[0], "pt-BR"),
  );
  ...
  for (const [, term] of rankedTerms) {
    ...
    if (
      selectedTermSets.some(
        (selected) =>
          [...terms].every((token) => selected.has(token)) ||
          [...selected].every((token) => terms.has(token)),
      )
    )
      continue;
  ...
  for (const sentence of sentences) {
    sentence.score =
      [...sentence.terms].reduce(
        (sum, token) => sum + (vocabulary.get(token)?.score ?? 0),
        0,
      ) / Math.sqrt(sentence.terms.size);
    if (
      /\b(?:é|são|consiste|significa|caracteriza|define|ocorre|permite|representa)\b/i.test(
        sentence.text,
      )
    )
      sentence.score *= 1.15;
    ...
    for (let index = 0; index < candidates.length; index++) {
      ...
      const similarity = Math.max(
        ...
        ...selected.map((previous) => {
          const overlap = [...candidate.terms].filter((token) =>
            previous.terms.has(token),
          ).length;
          return overlap / Math.min(candidate.terms.size, previous.terms.size);
        }),
      );
      if (similarity > 0.8 && rangeCounts.has(range)) continue;
      const rangeBoost = rangeCounts.has(range) ? 1 : 1.7;
      const score =
        (candidate.score * rangeBoost * (1 - 0.75 * similarity)) /
        (1 + (pageCounts.get(candidate.page) ?? 0) * 0.4);
      if (score > bestScore) {
        bestScore = score;
        bestIndex = index;
      }
  ...
  const partial =
    readablePages < pages.length
      ...
      : "";
  const notes = selected.map(
    (sentence) => `• ${sentence.text} (p. ${sentence.page})`,
  );
  ...
  for (const keyword of keywords) {
    ...
    const evidence = [...sentences]
      .filter(
        (sentence) =>
          !usedEvidence.has(sentence.index) &&
          tokens.every((token) => sentence.terms.has(token)),
      )
